...
//practicing same concept
let currentUsers = ["@ali", "#khan", "$jaffar", "123pilly"];
let newUsers = ["@ali", "#khan", "khani", "adnan"];
for (let i = 0; i < newUsers.length; i++) {
    if (newUsers[i].includes(currentUsers[i])) {
        console.log(` ${newUsers[i]}  user name is already exist .`);
    }
    else {
        console.log(` ${newUsers[i]} user name is available .`);
    }
}
